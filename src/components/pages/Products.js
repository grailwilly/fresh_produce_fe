...
const Products = ({ productItems, productBox, cardData, increaseQty, decreaseQty,updateQty }) => {
                ...
                productItems.map((e, i) => {
                  return (
                    <div key={i} className="flex flex-col items-center justify-center w-full max-w-lg mx-auto border border-teal p-5 rounded">
                      ...
                      <p className="text-blue-500 text-xs">{priceFormat(e.price)}</p>

                      <div className="custom-number-input h-10 w-32">
                        <div className="flex flex-row h-10 w-full rounded-lg relative bg-transparent mt-1">
                          <button data-action="decrement" onClick={() => decrease(e.id)} className="bg-gray-300 text-gray-600 hover:text-gray-700 hover:bg-gray-400 h-full w-20 rounded-l cursor-pointer outline-none">
                          ...
                          <button data-action="increment" onClick={() => add(e.id)} className="bg-gray-300 text-gray-600 hover:text-gray-700 hover:bg-gray-400 h-full w-20 rounded-r cursor-pointer">
                            <span className="m-auto text-2xl font-thin">+</span>
                          </button>
                          <span className='m-auto text-xs'>Box</span>
                        </div>
                      </div>
                      <span className="m-auto text-xs text-dk-white">{`10 ${e.qty_measurement} / box`}</span>
...
export default connect(MapToStateProps, { products, productBox, increaseQty, decreaseQty, updateQty })(Products)